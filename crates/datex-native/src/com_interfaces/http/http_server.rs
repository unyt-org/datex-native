...
use datex_core::{
    ...
    network::{
        ...
        com_interfaces::{
            ...
            default_setup_data::http::http_server::HTTPServerInterfaceSetupData,
        },
    },
};
...
impl HTTPServerInterfaceSetupDataNative {
    async fn create_interface(
        ...
        let mut server =
            Server::http(&addr.to_string(), false).await.map_err(|e| {
        ...
        Ok(ComInterfaceConfiguration::new_multi_socket(
            ComInterfaceProperties {
                ...
                connectable_interfaces:
                    HTTPServerInterfaceSetupData::get_clients_setup_data(
                ...
                while let Some(request) = server.next().await {
                    ...
                    yield Ok(SocketConfiguration::new_in_out(
                        ...
                        SendCallback::new_sync_once(move |block: DXBBlock| {
                            let response =
                                Response::from_data(block.to_bytes());
                            request
                                .respond(response)
                                .map_err(|e| {
                                    println!(
                                        "HTTP response send error: {:#?}",
                                        e
                                    );
