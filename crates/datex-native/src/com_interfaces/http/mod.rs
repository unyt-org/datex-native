...
mod tests {
    use crate::com_interfaces::{
        http::{
            http_client::HTTPClientInterfaceSetupDataNative,
            http_server::HTTPServerInterfaceSetupDataNative,
        },
        tests::{
            test_receive_block, test_send_block_async_callback,
            test_send_block_sync_once_callback,
        },
    };
    use datex_core::{
    ...
    use datex_core::{
        network::com_interfaces::default_setup_data::http::{
            http_client::HTTPClientInterfaceSetupData,
            http_server::HTTPServerInterfaceSetupData,
        },
    ...
    async fn test_connect_and_communicate() {
        ...
        let mut client_interface_configuration =
            HTTPClientInterfaceSetupDataNative(HTTPClientInterfaceSetupData {
        ...
        assert_eq!(client_interface_configuration.properties.channel, "http");
        assert_eq!(
        ...
        assert_eq!(server_interface_configuration.properties.channel, "http");
        assert_eq!(
        ...
        join!(
            async move {
                ...
                let server_socket = async_next_pin_box(
                ...
                test_receive_block(
                    &mut server_socket.iterator.unwrap(),
                    message_clone,
                )
                .await;
                ...
                test_send_block_sync_once_callback(
                    &server_socket.send_callback,
                    response_message_clone.clone(),
                );
            },
            async move {
                ...
                test_send_block_async_callback(
                    &client_socket.send_callback,
                    message,
                )
                .await;
                ...
                test_receive_block(
                    &mut client_socket.iterator.unwrap(),
                    response_message,
                )
                .await;
            }
