...
pub mod tests {
    ...
    pub async fn test_client_server_sockets(
        ...
        test_send_block_async_callback(
            &client_socket_configuration.send_callback,
            message.clone(),
        )
        .await;
        ...
        test_receive_block(
            &mut server_socket_configuration.iterator.unwrap(),
            message,
        )
        .await;
        ...
        test_send_block_async_callback(
            &server_socket_configuration.send_callback,
        ...
        test_receive_block(
            &mut client_socket_configuration.iterator.unwrap(),
            response_message,
        )
        .await;
    }
    ...
    pub async fn test_client_server_interfaces(
        ...
        let mut server_socket_iterator =
            server_interface_configuration.new_sockets_iterator;
        let server_socket =
            async_next_pin_box(&mut server_socket_iterator).await;
        assert!(server_socket.is_some());
        let server_socket = server_socket.unwrap();
        assert!(server_socket.is_ok());
        let server_socket = server_socket.unwrap();

        let mut client_socket_iterator =
            client_interface_configuration.new_sockets_iterator;
        let client_socket =
            async_next_pin_box(&mut client_socket_iterator).await;
    ...
    pub(crate) async fn test_send_block_async_callback(
    ...
    /// Test utility function to send a block using the provided send callback.
    /// Asserts that the callback is asynchronous and successfully sends the block.
    pub(crate) fn test_send_block_sync_once_callback(
        send_callback: &Option<SendCallback>,
        block: DXBBlock,
    ) {
        match send_callback {
    ...
    pub(crate) async fn test_receive_block(
        block_iterator: &mut SocketDataIterator,
        matches_block: DXBBlock,
    ) {
